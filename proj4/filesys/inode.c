...
static bool
read_block (struct inode* inode, off_t offset, bool is_write, struct cache_line **ce_result)
{
  ...
  while(true)
  {
    ...
    if (*next_sector != 0)
    {
      ...
      if (this_level == level)
      {
        ...
        if ((this_level == 0 && sector_offs[this_level] < (off_t)NUM_DATA_SECTOR - 1) 
        || (this_level > 0 && sector_offs[this_level] < (off_t)NUM_BLOCK_PTR_PER_INODE - 1))
        {
