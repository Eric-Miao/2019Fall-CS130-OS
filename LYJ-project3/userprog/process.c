...
process_execute (const char *file_name) 
{
  ...
  tmp_name=strtok_r(tmp_name," ",&save);



  exec.file_name = fn_copy;

  tid = thread_create (tmp_name, PRI_DEFAULT, start_process, &exec);
  if (tid != TID_ERROR)
    {
      struct thread * tt=get_thread(tid);
      sema_down(&tt->start_wait);
      if (exec.success)
        list_push_back (&thread_current ()->list_of_sons, &exec.wait_status->elem);
      else 
        tid = TID_ERROR;
    }
...
start_process (void *exec_)
{
  ...
  if (!success){
    ...
    sema_up(&thread_current ()->start_wait);
    unusual_exit(-1);
  }
  sema_up(&thread_current ()->start_wait);
...
process_wait (tid_t child_tid) 
{
  ...
  //get the son thread
  struct thread *son=get_thread(child_tid);

  for (e = list_begin (&cur->list_of_sons); e != list_end (&cur->list_of_sons);
       e = list_next (e)) 
    {
      ...
      if (cs->tid == child_tid) 
        {
          int exit_code;
          list_remove (e);
          if(!(son==NULL||son->status==THREAD_DYING||son->save_data_to_parent)){
            son->waited_by_parent=true;
            sema_down(&son->parent->wait_done);
          }
...
process_exit (void)
{
  ...
  if (pd != NULL) 
    {
      printf ("%s: exit(%d)\n", cur->name, cur->end_status);

      if (cur->wait_status != NULL) 
        {
          struct wait_status *cs = cur->wait_status;
          cs->exit_code = cur->end_status;
        }

      cur->save_data_to_parent=true;
      if(cur->parent != NULL&&cur->waited_by_parent){
        sema_up(&cur->parent->wait_done);
      }

      //free the list for the son 
      while(!list_empty(&cur->list_of_sons)){
        free(list_entry(list_pop_front(&cur->list_of_sons),struct son_list_elem,elem));
      }
      ...
      if(cur->file_for_this_thread!=NULL){
        file_allow_write(cur->file_for_this_thread);
        file_close(cur->file_for_this_thread);
      }
...
load (const char *cmd_line, void (**eip) (void), void **esp) 
{
  ...
  cmd = strchr (file_name, ' ');
  if (cmd != NULL){
    *cmd = '\0';
  }

  /* Open executable file. */
  t->file_for_this_thread = file = filesys_open (file_name);
  if (file == NULL) 
    {
      printf ("load: %s: open failed\n", file_name);
      goto done; 
    }
  file_deny_write (t->file_for_this_thread);
...
//passing the arguments to the stack
static bool
...
setup_stack (const char *cmd_line, void **esp) 
{
  ...
  if (page != NULL) 
    {
      page->frame =frame_alloc_lock(page);
      if (page->frame != NULL)
        {
          bool ok;
