  ...
  if (pd != NULL) 
    {
      ...
      {
          struct loaded_file *f = list_entry (list_pop_front(&cur->file_list),
                       struct loaded_file, file_elem);
          file_close(f->file);
          free(f);
      }
...
int 
GetRetFromSonsList(struct thread *t,tid_t tid)
{
    struct list_elem *e;
    int ret=-1;
    ...
    for(e=list_begin(&t->sons_ret);e!=list_end(&t->sons_ret);e=list_next(e))
    {
        ...
        if(target_son->tid==tid)
        {
            ret=target_son->exit_status;
            target_son->exit_status=-1;     
            break;
        }
    }
    return ret;
